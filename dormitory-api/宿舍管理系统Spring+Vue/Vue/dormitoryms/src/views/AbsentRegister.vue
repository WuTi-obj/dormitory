<template>
    <div style="margin-top: 60px;margin-left:330px;width: 300px;height: 500px;border: 0px solid red;" >
        <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="100px" class="demo-ruleForm">
            ...
            <el-form-item label="选择日期" prop="createDate">
                <el-date-picker
                        style="width: 200px"
    ...
    export default {
        ...
        methods: {
            buildingChange(){
				...
				// console.log("宿管id",_this.admin.id);
				// axios.get('http://localhost:8085/absent/findBuildingByAdminId/'+_this.admin.id).then(function (resp1) {
				// 	console.log(resp1.data.data);
				// 	_this.ruleForm.buildingName = resp1.data.data.name
				// 	// _this.ruleForm.buildingId = resp1.data.data.id
				// 	console.log("楼宇name",_this.ruleForm.buildingName);
				// 	console.log("楼宇id",_this.ruleForm.buildingId);
				// })
                axios.get('http://localhost:8085/absent/findDormitoryByBuildingId/'+_this.ruleForm.buildingId).then(function (resp) {
            ...
            dormitoryChange(){
                const _this = this
                axios.get('http://localhost:8085/absent/findStudentByDormitoryId/'+_this.ruleForm.dormitoryId).then(function (resp) {
            ...
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        axios.post('http://localhost:8085/absent/save',_this.ruleForm).then(function (resp) {
                            if(resp.data.code == 0){
                                _this.$alert('缺寝登记成功', '', {
                                    confirmButtonText: '确定',
                                    callback: action => {
                                        _this.$router.push('/absentDormitoryRecord')
                                    }
                                });
                            }
                        })
        ...
        created() {
            const _this = this
            axios.get('http://localhost:8085/absent/buildingList').then(function (resp) {
