...
<script>
    export default {
        ...
        methods: {
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        axios.put('http://localhost:8085/dormitoryAdmin/update',_this.ruleForm).then(function (resp) {
                            if(resp.data.code == 0){
        ...
        created() {
            const _this = this
            axios.get('http://localhost:8085/dormitoryAdmin/findById/'+_this.$route.query.id).then(function (resp) {
