...
<script>
    export default {
        ...
        methods:{
            submitForm(formName) {
                ...
                this.$refs[formName].validate((valid) => {
                    if (valid) {
                        ...
                        axios.get('http://localhost:8085/dormitory/search',{params:_this.ruleForm}).then(function (resp) {
            ...
            page(currentPage){
                ...
                if(_this.ruleForm.value == ''){
                    axios.get('http://localhost:8085/dormitory/list/'+currentPage+'/'+_this.pageSize).then(function (resp) {
                        _this.tableData = resp.data.data.data
                        _this.total = resp.data.data.total
                    })
                } else {
            ...
            del(row) {
                ...
                this.$confirm('确认删除【'+row.name+'】吗？', '提示', {
                    ...
                    axios.delete('http://localhost:8085/dormitory/deleteById/'+row.id).then(function (resp) {
                        if(resp.data.code==0){
        ...
        created() {
            const _this = this
            axios.get('http://localhost:8085/dormitory/list/1/'+_this.pageSize).then(function (resp) {
                _this.tableData = resp.data.data.data
                _this.total = resp.data.data.total
            })
        }
