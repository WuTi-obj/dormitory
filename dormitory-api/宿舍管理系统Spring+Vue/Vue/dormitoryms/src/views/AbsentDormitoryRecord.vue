			...
			<el-table-column label="操作" width="130">
			    <template slot-scope="scope">
			        <el-button
			                size="mini"
			                type="danger"
			                @click="absentout(scope.row)">销假</el-button>
			    </template>
			</el-table-column>
        </el-table>
    ...
    export default {
        ...
        methods:{
			...
			absentout(row) {
			    ...
			    this.$confirm('确认销假【'+row.studentName+'】吗？', '提示', {
					confirmButtonText: '确定',
					cancelButtonText: '取消',
					type: 'warning'
				}).then(() => {
					axios.put('http://localhost:8085/absent/deleteById/'+row.id).then(function (resp) {
						if(resp.data.code == 0){
							_this.$alert('【'+row.studentName+'】已销假', '', {
								confirmButtonText: '确定',
								callback: action => {
									location.reload()
								}
